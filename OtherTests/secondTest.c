...
void inittoken() { 
  ...
  char *textLabel;
  rv = (lib -> C_Initialize)(NULL_PTR);//Indica a SoftHSM que esta sera una aplicacion cryptoki
  if(rv != CKR_OK) {
 ...
 rv = (lib -> C_InitToken)(slotWithToken, soPIN, sizeof(soPIN) - 1, paddedLabel);
 //Inicializa el token, si no le hace las operaciones de abajo igual funcionan, excepto si es la primera vez
  ...
  rv = (lib -> C_OpenSession)(slotWithToken, CKF_SERIAL_SESSION | CKF_RW_SESSION, NULL_PTR, NULL_PTR, &hSession);//Abre una sesion en el slot 1, con las opciones de serial (es pmHSM paralelo?) y de RW. Y Guarde el handle session en hSession, sucede lo mismo en checks
  ...
  rv = (lib -> C_Login)(hSession, CKU_SO, soPIN, sizeof(soPIN) - 1);
  ...
  rv = (lib -> C_InitPIN)(hSession, userPIN, sizeof(userPIN) - 1);
  if(rv != CKR_OK) {
    printf("Error: Could not initialize the user PIN.\n");
    exit(1);
  }
  
  rv = (lib -> C_Finalize)(NULL_PTR);
  if(rv != CKR_OK) {
...
int main(int argc, char **argv)
{
	...
	if (argc != 2)
	{
		...
		printf("./secondTest /usr/local/lib/softhsm/libsofthsm.so\n");
		exit(1);
	}
...
void runInitCheck(unsigned int counter) { //INICIALIZA Y FINALIZA MUCHAS VECES TESTEANDO CASOS QUE NO FUNCIONAN TAMBIEN
  ...
  for(i = 0; i < counter; i++) {
    ...
    rv = (lib -> C_Finalize)((CK_VOID_PTR)1);
    ...
    rv = (lib -> C_Initialize)((CK_VOID_PTR)&InitArgs);
...
void runInfoCheck(unsigned int counter) {
  ...
  for(i = 0; i < counter; i++) {
    ...
    rv = (lib -> C_GetInfo)(NULL_PTR);
    assert(rv == CKR_CRYPTOKI_NOT_INITIALIZED);
    rv = (lib -> C_GetSlotList)(CK_FALSE, NULL_PTR, NULL_PTR);
    assert(rv == CKR_CRYPTOKI_NOT_INITIALIZED);
    rv = (lib -> C_GetSlotInfo)(slotInvalid, NULL_PTR);
    assert(rv == CKR_CRYPTOKI_NOT_INITIALIZED);
    rv = (lib -> C_GetTokenInfo)(slotInvalid, NULL_PTR);
    assert(rv == CKR_CRYPTOKI_NOT_INITIALIZED);
    rv = (lib -> C_GetMechanismList)(slotInvalid, NULL_PTR, NULL_PTR);
    assert(rv == CKR_CRYPTOKI_NOT_INITIALIZED);
    rv = (lib -> C_GetMechanismInfo)(slotInvalid, CKM_VENDOR_DEFINED, NULL_PTR);
    assert(rv == CKR_CRYPTOKI_NOT_INITIALIZED);//Se caen pues necesitan que el cryptoki este inicualizado

    /* C_GetFunctionList */
    
    rv = (lib -> C_GetFunctionList)(NULL_PTR);
    ...
    rv = (lib -> C_GetFunctionList)(&ckFuncList);
    ...
    rv = (lib -> C_GetSlotList)(CK_FALSE, NULL_PTR, &ulSlotCount);
    assert(rv == CKR_OK);

    pSlotList = (CK_SLOT_ID_PTR)malloc(ulSlotCount * sizeof(CK_SLOT_ID));//Reserva memoria para la lista de slots
    ulSlotCount = 0;
    rv = (lib -> C_GetSlotList)(CK_FALSE, pSlotList, &ulSlotCount);
    assert(rv == CKR_BUFFER_TOO_SMALL);
    rv = (lib -> C_GetSlotList)(CK_FALSE, pSlotList, &ulSlotCount);
    assert(rv == CKR_OK);
    free(pSlotList);

    rv = (lib -> C_GetSlotList)(CK_TRUE, NULL_PTR, &ulSlotCount);
    assert(rv == CKR_OK);
    pSlotList = (CK_SLOT_ID_PTR)malloc(ulSlotCount * sizeof(CK_SLOT_ID));
    ulSlotCount = 0;
    rv = (lib -> C_GetSlotList)(CK_TRUE, pSlotList, &ulSlotCount);
    assert(rv == CKR_BUFFER_TOO_SMALL);
    rv = (lib -> C_GetSlotList)(CK_TRUE, pSlotList, &ulSlotCount);
    assert(rv == CKR_OK);
    ...
    rv = (lib -> C_GetSlotInfo)(slotInvalid, &slotInfo);
    assert(rv == CKR_SLOT_ID_INVALID);//Falla pues el slot especificado no existe
    rv = (lib -> C_GetSlotInfo)(slotWithToken, &slotInfo);
    assert(rv == CKR_OK);
    ...
    rv = (lib -> C_GetTokenInfo)(slotInvalid, &tokenInfo);
    assert(rv == CKR_SLOT_ID_INVALID);//Falla pues el slot especificado no existe
    rv = (lib -> C_GetTokenInfo)(slotWithNoToken, &tokenInfo);
    assert(rv == CKR_TOKEN_NOT_PRESENT);//Falla pues el slot no tiene un token
    rv = (lib -> C_GetTokenInfo)(slotWithToken, &tokenInfo);
    assert(rv == CKR_OK);
    ...
    rv = (lib -> C_GetMechanismInfo)(slotInvalid, CKM_VENDOR_DEFINED, NULL_PTR);
    assert(rv == CKR_SLOT_ID_INVALID);//Falla pues el slot no es valido
    rv = (lib -> C_GetMechanismInfo)(slotWithToken, CKM_VENDOR_DEFINED, NULL_PTR);
    assert(rv == CKR_ARGUMENTS_BAD);//Falla pues el tercer argumento debe se un puntero a un CK_MECHANISM_INFO
    rv = (lib -> C_GetMechanismInfo)(slotWithToken, CKM_VENDOR_DEFINED, &info);
    assert(rv == CKR_MECHANISM_INVALID);//Falla pues el token no puede ser usado en el token
    rv = (lib -> C_GetMechanismInfo)(slotWithToken, CKM_RSA_PKCS_KEY_PAIR_GEN, &info);
    assert(rv == CKR_OK);
    rv = (lib -> C_GetMechanismInfo)(slotWithToken, CKM_RSA_PKCS, &info);
    assert(rv == CKR_OK);
    rv = (lib -> C_GetMechanismInfo)(slotWithToken, CKM_MD5, &info);
    assert(rv == CKR_OK);
    rv = (lib -> C_GetMechanismInfo)(slotWithToken, CKM_RIPEMD160, &info);
    assert(rv == CKR_OK);
    rv = (lib -> C_GetMechanismInfo)(slotWithToken, CKM_SHA_1, &info);
    assert(rv == CKR_OK);
    rv = (lib -> C_GetMechanismInfo)(slotWithToken, CKM_SHA256, &info);
    assert(rv == CKR_OK);
    rv = (lib -> C_GetMechanismInfo)(slotWithToken, CKM_SHA384, &info);
    assert(rv == CKR_OK);
    rv = (lib -> C_GetMechanismInfo)(slotWithToken, CKM_SHA512, &info);
    assert(rv == CKR_OK);
    rv = (lib -> C_GetMechanismInfo)(slotWithToken, CKM_MD5_RSA_PKCS, &info);
    assert(rv == CKR_OK);
    rv = (lib -> C_GetMechanismInfo)(slotWithToken, CKM_RIPEMD160_RSA_PKCS, &info);
    assert(rv == CKR_OK);
    rv = (lib -> C_GetMechanismInfo)(slotWithToken, CKM_SHA1_RSA_PKCS, &info);
    assert(rv == CKR_OK);
    rv = (lib -> C_GetMechanismInfo)(slotWithToken, CKM_SHA256_RSA_PKCS, &info);
    assert(rv == CKR_OK);
    rv = (lib -> C_GetMechanismInfo)(slotWithToken, CKM_SHA384_RSA_PKCS, &info);
    assert(rv == CKR_OK);
    rv = (lib -> C_GetMechanismInfo)(slotWithToken, CKM_SHA512_RSA_PKCS, &info);
...
void runSessionCheck(unsigned int counter) {
  ...
  for(i = 0; i < counter; i++) {
    ...
    rv = (lib -> C_OpenSession)(slotInvalid, 0, NULL_PTR, NULL_PTR, NULL_PTR);
    assert(rv == CKR_CRYPTOKI_NOT_INITIALIZED);//Falla pues aun no es una aplicacion cryptoki
    rv = (lib -> C_CloseSession)(CK_INVALID_HANDLE); //Trata de cerrar sesion con un 0 como handle
    assert(rv == CKR_CRYPTOKI_NOT_INITIALIZED);
    rv = (lib -> C_CloseAllSessions)(slotInvalid);
    assert(rv == CKR_CRYPTOKI_NOT_INITIALIZED);
    rv = (lib -> C_GetSessionInfo)(CK_INVALID_HANDLE, NULL_PTR);
    assert(rv == CKR_CRYPTOKI_NOT_INITIALIZED);
    ...
    rv = (lib -> C_OpenSession)(slotWithNoToken, 0, NULL_PTR, NULL_PTR, NULL_PTR);
    assert(rv == CKR_TOKEN_NOT_PRESENT);//Falla pues el slot no tiene un token
    rv = (lib -> C_OpenSession)(slotWithNotInitToken, 0, NULL_PTR, NULL_PTR, NULL_PTR);
    assert(rv == CKR_TOKEN_NOT_RECOGNIZED);//Falla pues se le da un slot que tiene un token pero no esta inicializado
    rv = (lib -> C_OpenSession)(slotWithToken, 0, NULL_PTR, NULL_PTR, NULL_PTR);
    assert(rv == CKR_SESSION_PARALLEL_NOT_SUPPORTED);//Falla pues inteneta abrir una sesion sin el valor CKF_SERIAL_SESSION
    rv = (lib -> C_OpenSession)(slotWithToken, CKF_SERIAL_SESSION, NULL_PTR, NULL_PTR, NULL_PTR);
    ...
    rv = (lib -> C_OpenSession)(slotWithToken, CKF_SERIAL_SESSION, NULL_PTR, NULL_PTR, &hSession[0]);
    assert(rv == CKR_OK);

    /* C_CloseSession */

    rv = (lib -> C_CloseSession)(CK_INVALID_HANDLE);
    assert(rv == CKR_SESSION_HANDLE_INVALID);//Falla pues se cierra la session de un handle(0) invalido
    rv = (lib -> C_CloseSession)(hSession[0]);
    assert(rv == CKR_OK);

    /* C_CloseAllSessions */

    rv = (lib -> C_OpenSession)(slotWithToken, CKF_SERIAL_SESSION, NULL_PTR, NULL_PTR, &hSession[1]);
    assert(rv == CKR_OK);
    rv = (lib -> C_OpenSession)(slotWithToken, CKF_SERIAL_SESSION, NULL_PTR, NULL_PTR, &hSession[2]);
    assert(rv == CKR_OK);
    rv = (lib -> C_OpenSession)(slotWithToken, CKF_SERIAL_SESSION, NULL_PTR, NULL_PTR, &hSession[3]);
    assert(rv == CKR_OK);
    rv = (lib -> C_OpenSession)(slotWithToken, CKF_SERIAL_SESSION, NULL_PTR, NULL_PTR, &hSession[4]);
    assert(rv == CKR_OK);
    rv = (lib -> C_CloseSession)(hSession[3]);
    assert(rv == CKR_OK);
    rv = (lib -> C_CloseAllSessions)(slotInvalid);
    assert(rv == CKR_SLOT_ID_INVALID);//Falla pues se cierra la session de un handle(0) invalido
    rv = (lib -> C_CloseAllSessions)(slotWithNoToken);
    assert(rv == CKR_OK);
    rv = (lib -> C_CloseSession)(hSession[2]);
    assert(rv == CKR_OK);
    rv = (lib -> C_CloseAllSessions)(slotWithToken);//Y test de interoperabilidad?
    assert(rv == CKR_OK);
    ...
    rv = (lib -> C_OpenSession)(slotWithToken, CKF_SERIAL_SESSION, NULL_PTR, NULL_PTR, &hSession[0]);
    assert(rv == CKR_OK);
    rv = (lib -> C_OpenSession)(slotWithToken, CKF_SERIAL_SESSION | CKF_RW_SESSION, NULL_PTR, NULL_PTR, &hSession[1]);
    assert(rv == CKR_OK);
    rv = (lib -> C_GetSessionInfo)(hSession[0], NULL_PTR);
    assert(rv == CKR_ARGUMENTS_BAD);//Falla pues necesita una referencia a un CK_SESSION_INFO en donde se guardara la info
    rv = (lib -> C_GetSessionInfo)(hSession[0], &info);
    assert(rv == CKR_OK);
    rv = (lib -> C_GetSessionInfo)(hSession[1], &info);
...
void runUserCheck(unsigned int counter) {
  ...
  for(i = 0; i < counter; i++) {
    ...
    rv = (lib -> C_Login)(CK_INVALID_HANDLE, 9999, NULL_PTR, 0);
    assert(rv == CKR_CRYPTOKI_NOT_INITIALIZED);
    rv = (lib -> C_Logout)(CK_INVALID_HANDLE);
    ...
    rv = (lib -> C_Login)(hSession[0], 9999, NULL_PTR, 0);
    assert(rv == CKR_ARGUMENTS_BAD);//
    rv = (lib -> C_Login)(hSession[0], 9999, userPIN, MIN_PIN_LEN - 1);
    assert(rv == CKR_PIN_INCORRECT);//Falla pues el largo es menor que el minimo especificado para el HSM
    rv = (lib -> C_Login)(hSession[0], 9999, userPIN, MAX_PIN_LEN + 1);
    assert(rv == CKR_PIN_INCORRECT);//Falla pues el largo es menor que el minimo especificado para el HSM
    rv = (lib -> C_Login)(hSession[0], 9999, userPIN, sizeof(userPIN) - 2);
    ...
    rv = (lib -> C_Login)(hSession[0], CKU_CONTEXT_SPECIFIC, userPIN, sizeof(userPIN) - 2);
    ...
    rv = (lib -> C_Login)(hSession[0], CKU_USER, userPIN, sizeof(userPIN) - 2);
    ...
    rv = (lib -> C_Login)(hSession[0], CKU_USER, userPIN, sizeof(userPIN) - 1);
    assert(rv == CKR_OK);
    rv = (lib -> C_Login)(hSession[0], CKU_CONTEXT_SPECIFIC, userPIN, sizeof(userPIN) - 1);
    ...
    rv = (lib -> C_Login)(hSession[1], CKU_SO, soPIN, sizeof(soPIN) - 2);
    ...
    rv = (lib -> C_CloseSession)(hSession[0]); //Se cierra la sesion R/O
    assert(rv == CKR_OK);
    ...
    rv = (lib -> C_Login)(hSession[1], CKU_SO, soPIN, sizeof(soPIN) - 2);
    assert(rv == CKR_PIN_INCORRECT);//Falla pues el largo del pin es diferente al largo de userPin, (-1 pues no incluye null character al final)
    rv = (lib -> C_Login)(hSession[1], CKU_SO, soPIN, sizeof(soPIN) - 1);
    assert(rv == CKR_OK);
    rv = (lib -> C_Login)(hSession[1], CKU_CONTEXT_SPECIFIC, soPIN, sizeof(soPIN) - 1);
    assert(rv == CKR_OK);
    rv = (lib -> C_Login)(hSession[1], CKU_USER, userPIN, sizeof(userPIN) - 2);
    ...
    ////ASI
    //rv = (lib -> C_Logout)(hSession[1]);
    //rv = (lib -> C_Login)(hSession[1], CKU_USER, userPIN, sizeof(userPIN) - 1);
    //assert(rv == CKR_OK);
    ...
    rv = (lib -> C_Login)(hSession[1], CKU_CONTEXT_SPECIFIC, userPIN, sizeof(userPIN)-1);
    rv = (lib -> C_Logout)(hSession[0]);
    rv = (lib -> C_Login)(hSession[1], CKU_SO, userPIN, sizeof(userPIN)-1);
...
void runRandomCheck(unsigned int counter) {
  ...
  for(i = 0; i < counter; i++) {
    ...
    rv = (lib -> C_SeedRandom)(CK_INVALID_HANDLE, NULL_PTR, 0);
    assert(rv == CKR_CRYPTOKI_NOT_INITIALIZED);
    rv = (lib -> C_GenerateRandom)(CK_INVALID_HANDLE, NULL_PTR, 0);
    ...
    rv = (lib -> C_SeedRandom)(hSession[0], NULL_PTR, 0);
    assert(rv == CKR_ARGUMENTS_BAD);//Falla, pues la funcion necesita de un seed
    rv = (lib -> C_SeedRandom)(hSession[0], seed, sizeof(seed));
    assert(rv == CKR_OK);
    ...
    rv = (lib -> C_GenerateRandom)(CK_INVALID_HANDLE, NULL_PTR, 0);
    assert(rv == CKR_SESSION_HANDLE_INVALID);//Falla pues se le da un handle de session invalido
    rv = (lib -> C_GenerateRandom)(hSession[0], NULL_PTR, 0);
    ...
    rv = (lib -> C_GenerateRandom)(hSession[0], randomData, 40);
    assert(rv == CKR_OK);
    rv = (lib -> C_Finalize)(NULL_PTR);
    assert(rv == CKR_OK);
...
void runGenerateCheck(unsigned int counter) {
  ...
  for(i = 0; i < counter; i++) {
    ...
    rv = (lib -> C_GenerateKeyPair)(CK_INVALID_HANDLE, NULL_PTR, NULL_PTR, 0, NULL_PTR, 0, NULL_PTR, NULL_PTR);
    assert(rv == CKR_CRYPTOKI_NOT_INITIALIZED);//Falla pues la aplicacion no es una aplicacion cryptoki
    rv = (lib -> C_DestroyObject)(CK_INVALID_HANDLE, CK_INVALID_HANDLE);
    assert(rv == CKR_CRYPTOKI_NOT_INITIALIZED);//Falla pues la aplicacion no es una aplicacion cryptoki
    rv = (lib -> C_CreateObject)(CK_INVALID_HANDLE, NULL_PTR, 0, NULL_PTR);
    assert(rv == CKR_CRYPTOKI_NOT_INITIALIZED);//Falla pues la aplicacion no es una aplicacion cryptoki
    ...
    rv = (lib -> C_GenerateKeyPair)(hSession[0], NULL_PTR, NULL_PTR, 0, NULL_PTR, 0, NULL_PTR, NULL_PTR);
    assert(rv == CKR_ARGUMENTS_BAD);// Falla pues necesita un puntero a un CK_MECHANISM QUE SERA EL ALGORITMO QUE HARA LA OPERACION
    rv = (lib -> C_GenerateKeyPair)(hSession[0], &mechanism, NULL_PTR, 0, NULL_PTR, 0, NULL_PTR, NULL_PTR);
    ...
    rv = (lib -> C_GenerateKeyPair)(hSession[0], &mechanism, publicKeyTemplate, 6, NULL_PTR, 0, NULL_PTR, NULL_PTR);
    ...
    rv = (lib -> C_GenerateKeyPair)(hSession[0], &mechanism, publicKeyTemplate, 6, privateKeyTemplate, 7, NULL_PTR, NULL_PTR);
    ...
    rv = (lib -> C_GenerateKeyPair)(hSession[0], &mechanism, publicKeyTemplate, 6, privateKeyTemplate, 7, &hPublicKey, NULL_PTR);
    assert(rv == CKR_ARGUMENTS_BAD);//Falla pues necesita un puntero a un CK_OBJECT_HANDLE que es el handle del objeto que tendra contendra llave publica
    rv = (lib -> C_GenerateKeyPair)(hSession[0], &mechanism, publicKeyTemplate, 6, privateKeyTemplate, 7, &hPublicKey, &hPrivateKey);
    ...
    rv = (lib -> C_GenerateKeyPair)(hSession[1], &mechanism, publicKeyTemplate, 6, privateKeyTemplate, 7, &hPublicKey, &hPrivateKey);
    assert(rv == CKR_MECHANISM_INVALID);//Falla pues CKM_VENDOR_DEFINED no es un tipo de mecanismo validp
    mechanism.mechanism = CKM_RSA_PKCS_KEY_PAIR_GEN;
    rv = (lib -> C_GenerateKeyPair)(hSession[1], &mechanism, publicKeyTemplate, 5, privateKeyTemplate, 7, &hPublicKey, &hPrivateKey);
    ...
    rv = (lib -> C_CreateObject)(hSession[1], pubTemplate, 5, &hCreateKey);
    assert(rv == CKR_USER_NOT_LOGGED_IN);//Falla pues hSession[1] no se encuentra logueado
    rv = (lib -> C_Login)(hSession[0], CKU_USER, userPIN, sizeof(userPIN) - 1);
    assert(rv == CKR_OK);
    rv = (lib -> C_CreateObject)(hSession[1], pubTemplate, 0, &hCreateKey);
    assert(rv == CKR_ATTRIBUTE_VALUE_INVALID);
    rv = (lib -> C_CreateObject)(hSession[1], pubTemplate, 1, &hCreateKey);
    assert(rv == CKR_ATTRIBUTE_VALUE_INVALID);
    rv = (lib -> C_CreateObject)(hSession[1], pubTemplate, 2, &hCreateKey);
    assert(rv == CKR_TEMPLATE_INCOMPLETE);
    rv = (lib -> C_CreateObject)(hSession[1], pubTemplate, 11, &hCreateKey);
    assert(rv == CKR_ATTRIBUTE_TYPE_INVALID);// Fallan pues el numero de atributos del template es diferente al especificado
    rv = (lib -> C_CreateObject)(hSession[1], pubTemplate, 10, &hCreateKey);
    assert(rv == CKR_OK);
    rv = (lib -> C_DestroyObject)(hSession[1], hCreateKey);
    assert(rv == CKR_OK);
...
void runObjectCheck(unsigned int counter) {
  ...
  for(i = 0; i < counter; i++) {
    ...
    rv = (lib -> C_FindObjectsInit)(CK_INVALID_HANDLE, NULL_PTR, 0);
    assert(rv == CKR_CRYPTOKI_NOT_INITIALIZED);// Falla pues la aplicacion aun no es una cryptoki
    rv = (lib -> C_FindObjects)(CK_INVALID_HANDLE, NULL_PTR, 0, NULL_PTR);
    assert(rv == CKR_CRYPTOKI_NOT_INITIALIZED);// Falla pues la aplicacion aun no es una cryptoki
    rv = (lib -> C_FindObjectsFinal)(CK_INVALID_HANDLE);
    assert(rv == CKR_CRYPTOKI_NOT_INITIALIZED);// Falla pues la aplicacion aun no es una cryptoki
    rv = (lib -> C_GetAttributeValue)(CK_INVALID_HANDLE, CK_INVALID_HANDLE, NULL_PTR, 0);
    assert(rv == CKR_CRYPTOKI_NOT_INITIALIZED);// Falla pues la aplicacion aun no es una cryptoki
    rv = (lib -> C_SetAttributeValue)(CK_INVALID_HANDLE, CK_INVALID_HANDLE, NULL_PTR, 0);
    assert(rv == CKR_CRYPTOKI_NOT_INITIALIZED);// Falla pues la aplicacion aun no es una cryptoki
    ...
    rv = (lib -> C_Login)(hSession[1], CKU_USER, userPIN, sizeof(userPIN) - 1);
    assert(rv == CKR_OK);
    rv = (lib -> C_GenerateKeyPair)(hSession[1], &mechanism, publicKeyTemplate, 6, privateKeyTemplate, 7, &hPublicKey, &hPrivateKey);//Genera un par de clave publica y privada guardandola con sus respectivos handles hPublicKey y hPrivateKey
    assert(rv == CKR_OK);
    rv = (lib -> C_Logout)(hSession[1]);//Se desloguea, por lo que hSession[0] y hSession[1] pasan a ser sesiones publicas
    assert(rv == CKR_OK);
    ...
    rv = (lib -> C_FindObjectsInit)(CK_INVALID_HANDLE, NULL_PTR, 0);
    assert(rv == CKR_SESSION_HANDLE_INVALID); //Falla pues se le da un handle de sesion invalido
    rv = (lib -> C_FindObjectsInit)(hSession[0], NULL_PTR, 1);
    ...
    rv = (lib -> C_FindObjectsInit)(hSession[0], searchTemplate, 1);
    assert(rv == CKR_OK);
    rv = (lib -> C_FindObjectsInit)(hSession[0], searchTemplate, 1);
    ...
    rv = (lib -> C_FindObjects)(hSession[0], &hObject, 0, NULL_PTR);
    ...
    rv = (lib -> C_FindObjects)(hSession[0], &hObject, 1, &ulObjectCount);
    assert(rv == CKR_OK);
    ...
    rv = (lib -> C_FindObjectsFinal)(CK_INVALID_HANDLE);
    assert(rv == CKR_SESSION_HANDLE_INVALID);// Falla pues el handle de sesion es invalido
    rv = (lib -> C_FindObjectsFinal)(hSession[1]);
    assert(rv == CKR_OPERATION_NOT_INITIALIZED);// Falla pues la sesion hSession[1] no ha iniciado ninguna busqueda
    rv = (lib -> C_FindObjectsFinal)(hSession[0]);//Para especificar que la operacion de busqueda ha finalizado
    assert(rv == CKR_OK);
    ...
    rv = (lib -> C_GetAttributeValue)(CK_INVALID_HANDLE, CK_INVALID_HANDLE, NULL_PTR, 0);
    assert(rv == CKR_SESSION_HANDLE_INVALID);// Falla pues el handle de sesion es invalido
    rv = (lib -> C_GetAttributeValue)(hSession[0], CK_INVALID_HANDLE, NULL_PTR, 0);
    assert(rv == CKR_OBJECT_HANDLE_INVALID);// Falla pues el handle de objeto es invalido
    rv = (lib -> C_GetAttributeValue)(hSession[0], hPrivateKey, NULL_PTR, 0);
    ...
    rv = (lib -> C_GetAttributeValue)(hSession[0], hPrivateKey, &getAttr, 1);
    ...
    rv = (lib -> C_SetAttributeValue)(CK_INVALID_HANDLE, CK_INVALID_HANDLE, NULL_PTR, 0);
    assert(rv == CKR_SESSION_HANDLE_INVALID);// Falla pues el handle de sesion es invalido
    rv = (lib -> C_SetAttributeValue)(hSession[0], CK_INVALID_HANDLE, NULL_PTR, 0);
    assert(rv == CKR_OBJECT_HANDLE_INVALID);// Falla pues el handle de objeto es invalido
    rv = (lib -> C_SetAttributeValue)(hSession[0], hPrivateKey, NULL_PTR, 0);
    ...
    rv = (lib -> C_SetAttributeValue)(hSession[1], hPrivateKey, NULL_PTR, 0);
    ...
    rv = (lib -> C_SetAttributeValue)(hSession[1], hPrivateKey, template2, 1);
    assert(rv == CKR_ATTRIBUTE_READ_ONLY);// Falla pues se quiere modificar la clase del objeto, el cual es un atributo r/o
    rv = (lib -> C_SetAttributeValue)(hSession[1], hPrivateKey, template1, 1);//Se modifica el CK_LABEL del objeto
    assert(rv == CKR_OK);
...
void runDigestCheck(unsigned int counter) {
  ...
  for(i = 0; i < counter; i++) {
    ...
    rv = (lib -> C_DigestInit)(CK_INVALID_HANDLE, NULL_PTR);
    assert(rv == CKR_CRYPTOKI_NOT_INITIALIZED); //Falla pues no es una aplicacion cryptoki
    rv = (lib -> C_Digest)(CK_INVALID_HANDLE, NULL_PTR, 0, NULL_PTR, NULL_PTR);
    assert(rv == CKR_CRYPTOKI_NOT_INITIALIZED); //Falla pues no es una aplicacion cryptoki
    rv = (lib -> C_DigestUpdate)(CK_INVALID_HANDLE, NULL_PTR, 0);
    assert(rv == CKR_CRYPTOKI_NOT_INITIALIZED); //Falla pues no es una aplicacion cryptoki
    rv = (lib -> C_DigestFinal)(CK_INVALID_HANDLE, NULL_PTR, NULL_PTR);
    assert(rv == CKR_CRYPTOKI_NOT_INITIALIZED); //Falla pues no es una aplicacion cryptoki
    ...
    digestLen = 0;
    rv = (lib -> C_Digest)(hSession[0], NULL_PTR, 0, digest, &digestLen);
    ...
    rv = (lib -> C_Digest)(hSession[0], data, sizeof(data)-1, digest, &digestLen);
    assert(rv == CKR_OK);
    rv = (lib -> C_Digest)(hSession[0], data, sizeof(data)-1, digest, &digestLen);
    ...
    rv = (lib -> C_DigestUpdate)(hSession[0], NULL_PTR, 0);
    ...
    rv = (lib -> C_DigestUpdate)(hSession[0], data, sizeof(data)-1);
    ...
    rv = (lib -> C_DigestFinal)(hSession[0], NULL_PTR, &digestLen);
    assert(rv == CKR_OK);
    digest = (CK_BYTE_PTR)malloc(digestLen);
    digestLen = 0;
    rv = (lib -> C_DigestFinal)(hSession[0], digest, &digestLen);
...
void runSignCheck(unsigned int counter) {
  ...
  for(i = 0; i < counter; i++) {
    ...
    rv = (lib -> C_SignInit)(CK_INVALID_HANDLE, NULL_PTR, CK_INVALID_HANDLE);
    assert(rv == CKR_CRYPTOKI_NOT_INITIALIZED);// Falla pues no es una aplicacion cryptoki
    rv = (lib -> C_Sign)(CK_INVALID_HANDLE, NULL_PTR, 0, NULL_PTR, NULL_PTR);
    assert(rv == CKR_CRYPTOKI_NOT_INITIALIZED);// Falla pues no es una aplicacion cryptoki
    rv = (lib -> C_SignUpdate)(CK_INVALID_HANDLE, NULL_PTR, 0);
    assert(rv == CKR_CRYPTOKI_NOT_INITIALIZED);// Falla pues no es una aplicacion cryptoki
    rv = (lib -> C_SignFinal)(CK_INVALID_HANDLE, NULL_PTR, NULL_PTR);
    assert(rv == CKR_CRYPTOKI_NOT_INITIALIZED);// Falla pues no es una aplicacion cryptoki
    ...
    rv = (lib -> C_Login)(hSession[1], CKU_USER, userPIN, sizeof(userPIN) - 1);
    assert(rv == CKR_OK);
    rv = (lib -> C_GenerateKeyPair)(hSession[1], &keyGenMechanism, publicKeyTemplate, 6, privateKeyTemplate, 7, &hPublicKey, &hPrivateKey);
    assert(rv == CKR_OK);
    rv = (lib -> C_Logout)(hSession[1]);//Desloguea las sesiones
    assert(rv == CKR_OK);
    ...
    rv = (lib -> C_SignInit)(hSession[0], NULL_PTR, CK_INVALID_HANDLE);
    assert(rv == CKR_KEY_HANDLE_INVALID);//Falla pues el handle de objeto llave que se le dio no es valido
    rv = (lib -> C_SignInit)(hSession[0], NULL_PTR, hPrivateKey);
    ...
    rv = (lib -> C_SignInit)(hSession[0], &mechanism, hPrivateKey);
    ...
    rv = (lib -> C_Sign)(hSession[0], data, sizeof(data)-1, pSignature, &length);
    assert(rv == CKR_OK);
    rv = (lib -> C_Sign)(hSession[0], data, sizeof(data)-1, pSignature, &length);
    ...
    rv = (lib -> C_SignUpdate)(CK_INVALID_HANDLE, NULL_PTR, 0);
    assert(rv == CKR_SESSION_HANDLE_INVALID);// Falla pues el handle de sesion es invalido
    rv = (lib -> C_SignUpdate)(hSession[0], NULL_PTR, 0);
    ...
    rv = (lib -> C_SignUpdate)(hSession[0], data, sizeof(data)-1);
    assert(rv == CKR_OK);
    ...
    rv = (lib -> C_SignFinal)(hSession[0], NULL_PTR, &length);
    assert(rv == CKR_OK);
    pSignature = (CK_BYTE_PTR)malloc(length);
    length = 0;
    rv = (lib -> C_SignFinal)(hSession[0], pSignature, &length);
